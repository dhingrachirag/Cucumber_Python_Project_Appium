...
class Driver():
    ...
    def start_appium(self, appium_port):
        ...
        self.appium_process = subprocess.Popen('appium -p {} -bp {}'.format(appium_port, str(int(appium_port)+1)), shell=True)
    ...
    def initialize_web_driver(self):
        ...
        for i in range(retries):
            try:
                ...
                #logging.info("wd:" + str(test_driver))
                ...
                if hasattr(appium_exception, 'msg'):
                    if 'MainActivity never started' in appium_exception.msg:
                        raise Exception(
                            'Application possibly crashed on start')
            if (i + 1) == retries:                
                Logger.error("AppiumFailure. Retries exhausted." + str(appium_exception))
                raise Exception(
                    "Cannot connect to Appium Server, Maximum retries {0} reached"
                    "last error failed with message : {1}".format(
            ...
            while (not (self.driver_utilities.is_element_present(target))):
                try:
                    ...
                    if (self.driver_utilities.is_element_present(self.ui_hash_map['account_type_alert'])):
                            if (shared == True) :
                                action_button = self.driver_utilities.wait_for_element(self.ui_hash_map['account_type_shared'])
                            else :
                                action_button = self.driver_utilities.wait_for_element(self.ui_hash_map['account_type_personal'])
                            self.driver_utilities.click_element(action_button)
